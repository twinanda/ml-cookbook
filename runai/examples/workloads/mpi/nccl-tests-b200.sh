...
runai mpi submit demo-nccl-test-16 -p $PROJECT_NAME \
    ...
    --master-args  "-np $(($NUM_WORKERS*$GPU_PER_NODE)) -mca coll ^hcoll --map-by ppr:$GPU_PER_NODE:node --allow-run-as-root bash -c '/opt/nccl_tests/build/all_reduce_perf -b 1024M -e 16G -f 2 -g 1'" \
